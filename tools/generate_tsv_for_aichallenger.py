...
def load_image_ids(split_name):
    ...
    split = []
    print "name of split is", split_name
    if split_name == 'aichallenger_train':
      for item in os.listdir("data/aichallenger_train"):
        if item.endswith(".jpg"):
          image_id = item.split(".")[0]
          filepath = os.path.join('data/aichallenger_train/', image_id + ".jpg")
          split.append((filepath,image_id))      
    elif split_name == 'aichallenger_validate':
      for item in os.listdir("data/aichallenger_validate"):
        if item.endswith(".jpg"):
          image_id = item.split(".")[0]
          filepath = os.path.join('data/aichallenger_validate', image_id + ".jpg")
          split.append((filepath,image_id))      
    elif split_name == 'aichallenger_test':
      for item in os.listdir("data/aichallenger_test"):
        if item.endswith(".jpg"):
          image_id = item.split(".")[0]
          filepath = os.path.join('data/aichallenger_test', image_id + ".jpg")
          split.append((filepath,image_id))      
    else:
      print 'Unknown split'
    print "split has %d files" % len(split)
    return split
...
def generate_tsv(gpu_id, prototxt, weights, image_ids, outfile):
    ...
    wanted_ids = set([image_id for filepath, image_id in image_ids])
    found_ids = set()
